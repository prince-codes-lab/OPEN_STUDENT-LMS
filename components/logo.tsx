...
export function Logo({ size = "md" }: LogoProps) {
  ...
  // Default SVG logo
  const DefaultLogo = (
    <svg width={s.container} height={s.container} viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  if (loading) {
    return <div className={`${sizeMap[size]} bg-[#4E0942] rounded-full animate-pulse`} />
  }

  return (
    <div className={`${sizeMap[size]} flex items-center justify-center bg-white rounded-full overflow-hidden`}>
      {logoUrl ? (
        <img src={logoUrl || "/placeholder.svg"} alt="Logo" className="h-full w-full object-contain p-2" />
      ) : (
        <div
          className={`${sizeMap[size]} bg-gradient-to-br from-[#4E0942] to-[#DD91D0] flex items-center justify-center`}
        >
          {DefaultLogo}
        </div>
      )}
