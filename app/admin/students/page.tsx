...
export default async function AdminStudentsPage() {
  ...
  const studentData = await Promise.all(
    students.map(async (student) => {
      const enrollmentCount = await Enrollment.countDocuments({ user_id: student._id })
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#DD91D0]/10 to-[#FF2768]/10">
        <div className="container mx-auto px-4 py-8">
          ...
          <Card className="border-2 border-[#4E0942]">
            <CardHeader>
              <CardTitle className="text-[#4E0942]">All Students ({studentData?.length || 0})</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                {studentData?.map((student) => (
                  <div key={student._id} className="p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
