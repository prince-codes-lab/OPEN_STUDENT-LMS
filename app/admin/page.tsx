...
export default async function AdminDashboard() {
  ...
  let stats = [
    { title: "Total Courses", value: 0, icon: BookOpen, color: "#4E0942" },
    { title: "Total Students", value: 0, icon: Users, color: "#FF2768" },
    { title: "Active Enrollments", value: 0, icon: TrendingUp, color: "#FEEB00" },
    { title: "Certificates Issued", value: 0, icon: Award, color: "#DD91D0" },
  ]

  try {
    await connectDB()
    const courses = await Course.find({}).maxTimeMS(5000).lean()
    const enrollments = await Enrollment.find({}).maxTimeMS(5000).lean()
    const certificates = await Certificate.find({}).maxTimeMS(5000).lean()
    const profiles = await User.find({}).maxTimeMS(5000).lean()

    stats = [
      { title: "Total Courses", value: courses?.length || 0, icon: BookOpen, color: "#4E0942" },
      { title: "Total Students", value: profiles?.length || 0, icon: Users, color: "#FF2768" },
      {
        title: "Active Enrollments",
        value: enrollments?.filter((e) => e.payment_status === "completed").length || 0,
        icon: TrendingUp,
        color: "#FEEB00",
      },
      { title: "Certificates Issued", value: certificates?.length || 0, icon: Award, color: "#DD91D0" },
    ]
  } catch (error) {
    console.log("[v0] Admin dashboard stats unavailable during build")
  }
