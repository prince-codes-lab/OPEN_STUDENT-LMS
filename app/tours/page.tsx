...
export default async function ToursPage() {
  ...
  try {
    await connectDB()
    tours = await Tour.find({ isActive: true }).sort({ date: 1 }).lean()
  } catch (error) {
                ...
                >
                  <CardHeader>
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <div className="text-right">
                        <div className="text-2xl font-bold text-[#FF2768]">₦{tour.priceNgn?.toLocaleString()}</div>
                        <div className="text-sm text-gray-600">${tour.priceUsd}</div>
                      </div>
                    </div>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <Button asChild className="w-full bg-[#4E0942] hover:bg-[#4E0942]/90 text-white font-semibold">
                      <Link href={`/enroll?tour=${tour._id}`}>Book Your Spot</Link>
                    </Button>
                  </CardContent>
                </Card>
              ))}
